...
const commentController = {
  ...
  getCommentsByProductId: async (req, res) => {
    try {
      const { id } = req.params;
      const comments = await Comment.find({ product_id: id }).populate('user_id', 'fullname email avatar');
  ...
  updateComment: async (req, res) => {
    try {
      ...
      res.status(200).json({ updateComment, message: 'Sửa bình luận thành công' });
    } catch (error) {
  ...
  getWithId: async (req, res) => {
    try {
      ...
      res.status(200).json({ message: 'Sửa bình luận thành công', userComment });
    } catch (error) {
      res.status(500).json({ mesage: error.message });
    }
  }
