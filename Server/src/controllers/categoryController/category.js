...
const categoryController = {
  ...
  deleteCategory: async (req, res) => {
    ...
    if (subCategories.length > 0) {
      return res.status(403).json({
        message: 'Không thể xóa danh mục vì nó chứa các danh mục con.'
      });
    }
    try {
  ...
  updateCategory: async (req, res) => {
    ...
    try {
      const updatedCategory = await CategoryModel.findByIdAndUpdate(id, { name, description, order }, { new: true });
  ...
  getProductWithCategory: async (req, res) => {
    ...
    try {
      ...
      const filteredCategories = categories[0].subcategories.map((subcategory) => {
        const productsForSubcategory = productsWithImages.filter(
          (product) => product.sub_category_id && product.sub_category_id.toString() === subcategory._id.toString()
        );
        return {
