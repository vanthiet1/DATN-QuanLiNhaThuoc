...
const AllBlog = () => {
  const { isLoading, isError, responsData: blogData, messsageError } = useFetch(blogServices.getAllBlogs);
              ...
              {blogData &&
                blogData.map((blog) => (
                  <tr key={blog._id} className='hover:bg-gray-100'>
                    <td className='px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-900'>{blog.code}</td>
                    <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
                      {formatsHelper.formatDate(blog?.image)}
                    </td>
                    <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
                      {formatsHelper.formatDate(blog?.title)}
                    </td>

                    <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
                      {formatsHelper.currency(blog?.description)}
                    </td>
