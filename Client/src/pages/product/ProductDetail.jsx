            ...
            <div className="w-full">
              ...
              <div className="flex justify-between w-[350px] max-md:w-full">
                <div className="flex gap-2 items-center">
                  <button className={`border-2 border-slate-200 w-[35px] text-[20px] font-bold ${quantityProductDetail > 1 && 'text-blue-500'}`} onClick={handleMinusQuantity}>-</button>
                  <input className="border-2 border-slate-200 w-[50px] px-1 text-center py-[3px] outline-none appearance-none" type="number" onChange={handleQuantityChange} value={quantityProductDetail} />
                  <button className={`border-2 border-slate-200  w-[35px] text-[20px]  font-bold ${quantityProductDetail > 0 && 'text-blue-500'}`} onClick={handlePlusQuantity}>+</button>
                </div>
                <div>
          ...
          <div className=" pt-4 gap-3 bg-[#FFFFFF] shadow p-10 rounded-[5px]">
            ...
            <div>
              <form onSubmit={handleSubmit(handlePostComment)}>
                ...
                />
                {errors.content && (
                  <p className="text-red-500 text-sm pl-2 pb-4">
                    {errors.content.message}
                  </p>
                )}
                <Button onClick={handlePostComment} className={`w-full duration-200  p-3 rounded-[50px] text-[#333] ${commentContent ? 'bg-green-700 duration-200 text-[#fff]' : 'bg-slate-200'}`}>Gửi nhận xét</Button>
              </form>
            </div>

            <div className="mt-[50px] h-[500px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200">
                    ...
                    <div className="flex justify-between items-center" key={comment._id} >
                      ...
                      {user?._id === comment?.user_id?._id && (
                        <div className="cursor-pointer" onClick={() => handleDeleteComment(comment?._id)}>
                          <AppIcons.TrashBinIcon addClassNames="text-red-500" />
                        </div>
                      )}
                    </div>
                  ))
